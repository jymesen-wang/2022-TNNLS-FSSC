function [result,labelnew,t,Rank,rp] = FSSC(X,B,U_final,label,alpha_u,alpha_l,isW)
% Input:
% —————— X: data matrix n*d
% —————— B: anchor graph matrix n*m
% —————— U_final: anchor data matrix m*d
...
for h = 1:c
    ...
    for l = 1:n
        if isW == 1
            ...
            fea(l) = (1 - rate*sum(Bdelta(I,:).*B(Ic,:)))*fea(l); % O(nc)
        end
    end
end
